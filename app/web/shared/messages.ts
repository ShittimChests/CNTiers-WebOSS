...
export const MESSAGES = {
  ...
  /**
   * 「重发」与「忘记密码」用这条而不是上面那条。
   *
   * 那两条路径无法断定真的发了信——邮箱可能不存在、可能已验证、也可能撞上
   * 账号级冷却，而这三种情况都必须与「已发送」不可区分（否则响应差异就是
   * 账号枚举信道）。既然如此，文案就必须对全部情况都成立，不能撒谎。
   */
  'auth.codeSentIfRegistered': '若该邮箱已注册，验证码已发送，请查收邮件',
